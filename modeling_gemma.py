...
class GemmaModel(nn.Module):
    ...
    def forward(
        ...
        # [btach_size, seq_len, hidden_size]
...
class PaliGemmaMultiModalProjector(nn.Module):
    ...
    def forward(self, image_features: torch.Tensor):
        # [batch_size, num_patches, embed_size] -> [batch_size, num_patches, projection_dim]
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def tie_weights(self):
        # similar to what i did in the SmallLM implementation,
        # we are making the weights of the embeddings layer equals to the weights of the linear projection layer outside the decoder
    ...
    def _merge_input_ids_with_image_features(
        ...
        # this means: whenever the text_mask_expanded is true, get the value from inputs_embeds, otherwise, get the value from final_embeddings, which is 0
        final_embeddings = torch.where(text_mask_expanded, inputs_embeds, final_embeddings)
        
        # insert image embeddings. we can't use torch.where because the seq len of scaled_image_features is not equal to the seq len of the final embedding
        final_embeddings = final_embeddings.masked_scatter(image_mask_expanded, scaled_image_features)
        
        # zero out paddikng tokens
        ...
        if kv_cache is None or kv_cache.num_items() == 0:
            ...
            # this only work when we have no padding
            ...
            # also in this case, we don't need to mak anything, sisnce each query should be able to attend all previous
        ...
        if kv_cache is not None and kv_cache.num_items() > 0:
            ...
            position_ids = (attention_mask.cumsum(-1)).masked_fill_((attention_mask.to(torch.bool) == 0), 1).to(device)
    ...
    def forward(
        ...
        # 1. extra th input embeddings
        # shape: (batch_size, seq_len, hidden_size)
        inputs_embeds = self.language_model.get_input_embedding()(input_ids)
        ...
        # [batch_size, num_patches, embed_size] 0> [batch_size, num_patches, hidden_size]
        image_features = self.multi_modal_projector(selected_image_features)
        
        # merge the embeddigns of the text tokens and the image tokens
